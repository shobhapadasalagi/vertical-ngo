...
class PurchaseOrderClassic(models.Model):
    ...
    def _default_state(self):
        if not self._context:
            return 'draft'
        elif self._context.get('draft_po'):
            return 'draftpo'
...
class PurchaseOrder(models.Model):
    ...
    type = fields.Selection(TYPE_SELECTION, 'Type', required=True,
        readonly=True, default=_default_type)
    ...
    pricelist_id = fields.Many2one('product.pricelist', 'Pricelist',
        readonly=True, required=True, states=STATES,
    ...
    def wkf_send_rfq(self):
        if not self._has_lines():
            raise exceptions.except_orm(
                _('Error!'),
                _('You cannot send a Request for Quotation without any \
                 product line.'))
...
class PurchaseOrderLine(models.Model):
    ...
    def onchange_product_id(self):
        ...
        order_type = self._context.get('order_type') or 'rfq'

        res = super(PurchaseOrderLine, self).onchange_product_id()

        if self.state == 'draft' and order_type == 'rfq':
            res.get['value'].update({'price_unit': 0.0})

        elif self.state in ('sent', 'draftbid', 'bid'):
