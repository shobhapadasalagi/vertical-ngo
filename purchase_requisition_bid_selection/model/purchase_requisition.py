...
class PurchaseRequisition(models.Model):
    ...
    bid_tendering_mode = fields.Selection(
        [('open', 'Open'),
         ('restricted', 'Restricted')], 'Call for Bids Mode', required=True,
        default='open',
    ...
    bid_receipt_mode = fields.Selection([
        ...
        default='open')
    req_incoterm_id = fields.Many2one('stock.incoterms', 
        'Requested Incoterms',
    ...
    req_incoterm_address = fields.Char(
        ...
        help="Incoterm Place of Delivery. International Commercial \
             Terms are a series of predefined commercial terms used \
              in international transactions.")
    req_payment_term_id = fields.Many2one('account.payment.term',
...
#                                   domain=[('type', '=', 'purchase')],
    ...
    # Report fields
    bid_ids = fields.One2many(comodel_name='purchase.order',
        inverse_name='requisition_id',
        domain=[('type', '=', 'bid')])
    eligible_bid_ids = fields.One2many(comodel_name='purchase.order',
        inverse_name='requisition_id',
        domain=[('type', '=', 'bid'),
    ...
    def _has_product_lines(self):
        ...
        for callforbids in self:
            if not callforbids.line_ids:
                raise except_orm(
                    _('Error!'),
                    _('You have to define some products before confirming'
                      'the call for bids.'))
        return True
